...
function PutForm({ label, setLabel, shared, setShared, expiry, setExpiry, error, onSave }: { label: string; setLabel: (value: string) => void; shared: boolean; setShared: (value: boolean) => void; expiry: string; setExpiry: (value: string) => void; error: string; onSave: () => void }) { return <div className="form-step"><span className="step-label">放入物品</span><h2>確認這件物品</h2><label htmlFor="item-label">物品名稱</label><div className="date-input"><span>✎</span><input id="item-label" value={label} onChange={event => setLabel(event.target.value)} placeholder="例如：鮮奶（使用者確認）" /></div><p className="rag-hint">HSV instance matcher 只辨識同一件物品，不會產生食物名稱。</p><span className="form-label">誰可以取用？</span><div className="permission-grid" role="group" aria-label="物品取用權限"><button className={!shared ? "selected" : ""} onClick={() => setShared(false)}><span>♙</span><strong>個人</strong><small>只有擁有者可取出</small></button><button className={shared ? "selected" : ""} onClick={() => setShared(true)}><span>♧</span><strong>共用</strong><small>已辨識成員可取出</small></button></div><label htmlFor="expiry">包裝期限 <em>選填</em></label><div className="date-input"><span>▣</span><input id="expiry" type="date" value={expiry} onChange={event => setExpiry(event.target.value)} /><button onClick={() => setExpiry("")}>不填</button></div>{error && <p className="rag-hint">{error}</p>}<div className="form-footer"><span>確認後才會呼叫相機與寫入 SQLite</span><button className="primary-button compact" onClick={onSave}>確認放入 →</button></div></div>; }
